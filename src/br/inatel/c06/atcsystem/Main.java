...
public class Main
{
    public static void main(String[] args)
    {
        ...
        //torre.processarVoos(); // tava assim antes
        Thread threadTorre = new Thread(torre);
        threadTorre.setName("TorreDeControle");
        threadTorre.start();

        // Aguardar um tempo para simular o sistema rodando
        try {
            Thread.sleep(5000); // Tempo para os voos acontecerem
        } catch (InterruptedException e) {
            e.printStackTrace();
        }

        // Pedir para a torre desligar
        torre.desligar();

        try {
            // Espera a thread da torre terminar antes de encerrar o programa
            threadTorre.join();
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
